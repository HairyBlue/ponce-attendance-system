...
function setupAdminEditStaffPage() {
  ...
  if (!staffId) {
    ...
    window.location.href = "/IAS/html/admin/admin_dashboard.html";
    return;
  }
  loadDepartments().then(() => {
...
function editStaff(id) {
  window.location.href = `/IAS/html/admin/admin_edit_staff.html?id=${id}`;
}
...
function editUser(id) {
  window.location.href = `/IAS/html/admin/admin_edit_user.html?id=${id}`;
}
...
function editEvent(eventId) {
  window.location.href = `/IAS/html/admin/admin_edit_event.html?id=${eventId}`;
}
...
function renderAttendanceRecords(records, studentId) {
  ...
  events.forEach((event) => {
    ...
    row.innerHTML = `
      <td class="border px-4 py-2">${records.student_id || studentId}</td>
      <td class="border px-4 py-2">${records.full_name || "N/A"}</td>
      <td class="border px-4 py-2">${records.course || "N/A"}</td>
