...
router.get("/:event_id", async (req, res) => {
  ...
  try {
    const query = `SELECT attendance.student_id FROM attendance WHERE attendance.event_id = ?;`; //ORDER BY attendance.student_id
...
router.get("/student/:student_id", async (req, res) => {
  ...
  try {
    // Retrieve events attended by the student in the given department,
    // including event id, name, and date.
    const [eventRows] = await db.query(
      `SELECT events.id AS event_id, events.name AS event_name, events.date AS event_date
       FROM attendance
    ...
    // // Fetch the student details via the external API.
    const response = await fetch(`${process.env.STUDENT_API_URL}/${student_id}`, {
